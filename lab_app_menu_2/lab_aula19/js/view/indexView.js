...
class IndexView {
    ...
    */
    ...
    createDishes(category, dishList, pedido) {
        ...
        let divPedido = `<h3>Pedido</h3> <h5 id='totalPedido'>0</h5><ul id='pedidoList' style="display: none"></ul> <h6><button class="btn btn-primary esconder" style="display: none" id="btnEsconde">Esconder pedidos</button></h6>`;
    ...
    listPedido(pedido) {
        ...
        if(pedido.total) {
            totalObj.innerHTML = `Total a pagar: R$` + pedido.total + `<h6><button class="btn btn-primary mostrar" id="btnMostra">Mostrar pedidos</button><h6/>`;
        }
        ...
        listaPedido.forEach((item) => {
            itemsObj.innerHTML += `<li>${item.name},  R$${item.price} <button id="${item.code}" class="btn btn-primary excluir">excluir</button></li>`;
        })
